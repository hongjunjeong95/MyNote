...
## 2단계: PGP 서명을 사용하여 Amazon ECS CLI 확인
...
### 2. 다음의 Amazon ECS PGP 퍼블릭 키 콘텐츠를 포함한 로컬 파일을 생성한 다음 가져온다.

```shell
$ mkdir keys
$ cd keys
$ nvim <public_key_filename>
=> nvim amazon-ecs-public-key.gpg
```
...
### 3. 다음 명령을 사용하여 Amazon ECS PGP 퍼블릭 키를 가져옵니다.

```shell
$ gpg --import <public_key_filename>
```

gpg usage : https://johngrib.github.io/wiki/gpg/



### 4. Amazon ECS CLI 서명을 다운로드한다. 서명은 `.asc` 확장자 파일에 저장된 ASCII 분리 PGP 서명이다. 서명 파일은 해당 실행 파일과 동일한 이름에 `.asc`가 추가되어 있다.
